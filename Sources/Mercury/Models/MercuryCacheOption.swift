...
public enum MercuryCacheOption: Equatable {
    /// Uses the system's shared URLCache. Not isolated.
    case shared
    /// Each client instance owns its own URLCache (size in bytes).
    case clientIsolated(
        memorySize: Int = MercuryCacheOption.defaultMemorySize,
        diskSize: Int = MercuryCacheOption.defaultDiskSize
    )
}

public extension MercuryCacheOption {
