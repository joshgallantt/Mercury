...
final class MockMercuryTests: XCTestCase {
    ...
    func test_givenStubbedGet_whenCalled_thenReturnsStub_andRecordsCall() async {
        ...
        // When
        let result = await mock.get(path: "/users/1", decodeTo: User.self)
    ...
    func test_givenStubbedPost_whenCalled_thenReturnsStub_andRecordsCall() async {
        ...
        // When
        let result = await mock.post(path: "/users", body: nil as User?, decodeTo: User.self)
    ...
    func test_givenStubbedPutPatchDelete_whenCalled_thenReturnsStubs() async {
        ...
        // When
        let putResult = await mock.put(path: "/users/3", body: updatedUser, decodeTo: User.self)
        let patchResult = await mock.patch(path: "/users/3", body: updatedUser, decodeTo: User.self)
        let deleteResult = await mock.delete(path: "/users/3", body: updatedUser, decodeTo: User.self)
    ...
    func test_givenStubbedFailure_whenCalled_thenReturnsFailure_andRecordsCall() async {
        // Given
        mock.stubFailure(method: .GET, path: "/fail", error: .server(statusCode: 500, data: nil), decodeTo: User.self)

        // When
        let result = await mock.get(path: "/fail", decodeTo: User.self)
    ...
    func test_givenNoStub_whenCalled_thenReturnsInvalidURLFailure() async {
        // When
        let result = await mock.get(path: "/notstubbed", decodeTo: User.self)
    ...
    func test_givenMultipleStubs_whenCalledWithDifferentPaths_thenEachStubIsIsolated() async {
        ...
        // When
        let aliceResult = await mock.get(path: "/users/1", decodeTo: User.self)
        let bobResult = await mock.get(path: "/users/2", decodeTo: User.self)
    ...
    func test_givenStubbedWithDelay_whenCalled_thenWaitsBeforeReturning() async {
        ...
        // When
        let start = Date()
        _ = await mock.get(path: "/delayed", decodeTo: User.self)
    ...
    func test_givenStubbedTwice_whenStubbedAgain_thenLastStubWins() async {
        ...
        // When
        let result = await mock.get(path: "/dup", decodeTo: User.self)
    ...
    func test_givenMultipleCalls_whenRecordedCallsQueried_thenShowsOrderAndParams() async {
        ...
        // When
        _ = await mock.get(path: "/a", decodeTo: User.self)
        _ = await mock.post(path: "/b", body: nil as User?, headers: ["X-Test": "1"], decodeTo: User.self)
    ...
    func test_givenReset_whenCalled_thenClearsAllState() async {
        ...
        _ = await mock.get(path: "/reset", decodeTo: User.self)
        ...
        let result = await mock.get(path: "/reset", decodeTo: User.self)
        switch result {
        case .success:
